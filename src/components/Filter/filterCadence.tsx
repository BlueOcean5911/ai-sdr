...
export default function FilterCadence() {
  ...
  const [ownedByOption, setOwnedByOption] = useState([]);
  ...
  const fetchUsers = () => {
    runService(
      ...
      getUsers,
      (users) => {
        const userOptions = users.map((user: UserModel) => ({
          value: user.id,
          label: user.firstName + " " + user.lastName,
        }));
        setOwnedByOption(userOptions);
        runService(
          undefined,
          getMe,
          (user) => {
