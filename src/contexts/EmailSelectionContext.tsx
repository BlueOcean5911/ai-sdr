...
const defaultEmails = [
  ...
  },
  {
    ...
    employees: 300,
    industry: "Consulting",
    keywords: ["emailership", "strategy", "consulting"],
  },
  {
    id: 7,
    name: "Grace Lee",
...
export const EmailSelectionContext = createContext<any>(undefined);

export const EmailSelectionProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [totalEmails, setTotalEmails] = useState<any[]>(defaultEmails);
  const [savedEmails, setSavedEmails] = useState<any[]>([]);
  const [selectedEmails, setSelectedEmails] = useState<any[]>([]);

  const handleSaveEmails = (emails: any[]) => {
    for (const email of emails) {
      const emailExists = savedEmails.find(
        (savedEmail) => savedEmail.id === email.id
      );
      if (!emailExists) {
        setSavedEmails([...savedEmails, ...emails]);
      }
    }
  };
