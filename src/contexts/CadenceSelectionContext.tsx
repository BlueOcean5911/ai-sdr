...
const defaultCadences = [
  ...
  },
  {
    ...
    employees: 300,
    industry: "Consulting",
    keywords: ["cadenceership", "strategy", "consulting"],
  },
  {
    id: 7,
    name: "Grace Lee",
...
export const CadenceSelectionContext = createContext<any>(undefined);

export const CadenceSelectionProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [totalCadences, setTotalCadences] = useState<any[]>(defaultCadences);
  const [savedCadences, setSavedCadences] = useState<any[]>([]);
  const [selectedCadences, setSelectedCadences] = useState<any[]>([]);

  const handleSaveCadences = (cadences: any[]) => {
    for (const cadence of cadences) {
      const cadenceExists = savedCadences.find(
        (savedCadence) => savedCadence.id === cadence.id
      );
      if (!cadenceExists) {
        setSavedCadences([...savedCadences, ...cadences]);
      }
    }
  };

  return (
    <CadenceSelectionContext.Provider
      value={{
        totalCadences,
        setTotalCadences,
        selectedCadences,
