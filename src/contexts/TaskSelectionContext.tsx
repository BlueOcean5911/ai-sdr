...
const defaultTasks = [
  ...
  },
  {
    ...
    employees: 300,
    industry: "Consulting",
    keywords: ["taskership", "strategy", "consulting"],
  },
  {
    id: 7,
    name: "Grace Lee",
...
export const TaskSelectionContext = createContext<any>(undefined);

export const TaskSelectionProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [totalTasks, setTotalTasks] = useState<any[]>(defaultTasks);
  const [savedTasks, setSavedTasks] = useState<any[]>([]);
  const [selectedTasks, setSelectedTasks] = useState<any[]>([]);

  const handleSaveTasks = (tasks: any[]) => {
    for (const task of tasks) {
      const taskExists = savedTasks.find(
