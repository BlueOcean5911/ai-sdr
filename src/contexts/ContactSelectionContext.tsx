...
const defaultContacts = [
  ...
  },
  {
    ...
    employees: 300,
    industry: "Consulting",
    keywords: ["contactership", "strategy", "consulting"],
  },
  {
    id: 7,
    name: "Grace Lee",
...
export const ContactSelectionContext = createContext<any>(undefined);

export const ContactSelectionProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [totalContacts, setTotalContacts] = useState<any[]>(defaultContacts);
  const [savedContacts, setSavedContacts] = useState<any[]>([]);
  const [selectedContacts, setSelectedContacts] = useState<any[]>([]);

  const handleSaveContacts = (contacts: any[]) => {
    for (const contact of contacts) {
      const contactExists = savedContacts.find(
